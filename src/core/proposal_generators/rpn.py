...
from src.core.box_regression import _dense_box_regression_loss, Box2BoxTransform
from src.config.config import Configurable
from src.core.anchor_generator import build_anchor_generator
...
class StandardRPNHead(tf.keras.layers.Layer):
    """
    ...
    def __init__(
        self,in_channels, num_anchors, box_dim=4, conv_dims=(-1,)
    ):
        """
        :param :in_channels (int): number of input feature channels. When using multiple
            input features, they must have the same number of channels.
        :param :num_anchors (int): number of anchors to predict for *each spatial position*
            on the feature map. The total number of anchors for each
            feature map will be `num_anchors * H * W`.
        :param :box_dim (int): dimension of a box, which is also the number of box regression
            predictions to make for each anchor. An axis aligned box has
            box_dim=4, while a rotated box has box_dim=5.
        :param :conv_dims (list[int]): a list of integers representing the output channels
        ...
        if len(conv_dims) == 1:
            ...
            self.conv = self._get_rpn_conv(out_channels)
            ...
            for k, conv_dim in enumerate(conv_dims):
                ...
                conv = self._get_rpn_conv( out_channels)
                self.conv.add(conv,f"conv{k}")
                cur_channels = out_channels
        # 1x1 conv for predicting objectness logits
        self.objectness_logits = Conv2D(num_anchors, kernel_size=1, strides=1)
        # 1x1 conv for predicting box2box transform deltas
        self.anchor_deltas = Conv2D(num_anchors * box_dim, kernel_size=1, strides=1)

    def _get_rpn_conv(self,out_channels):
        return Conv2D(
            out_channels,
            kernel_size=3,
            strides=1,
    ...
    def forward(self, features, training=None):
        ...
        pred_anchor_deltas = []
        for x in features:
            t = self.conv(x,training=training)
            pred_objectness_logits.append(self.objectness_logits(t))
            pred_anchor_deltas.append(self.anchor_deltas(t))
        return pred_objectness_logits, pred_anchor_deltas

@PROPOSAL_GENERATOR_REGISTRY.register()
class RPN(tf.keras.layers.Layer,metaclass=Configurable):
    """
    ...
    def __init__(
        ...
        Args:
            ...
            head: a module that predicts logits and regression deltas
                for each level from a list of per-level features
            anchor_generator: a module that creates anchors from a
    ...
    def _decode_proposals(self, anchors: List[Boxes], pred_anchor_deltas: List[tf.Tensor]):
        """
